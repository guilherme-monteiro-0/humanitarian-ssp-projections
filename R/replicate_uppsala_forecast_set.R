...
# Lags
conflict_w = conflict_w[order(conflict_w$Scenario, conflict_w$iso3, conflict_w$year),]
...
conflict_w_spatial = foreach(i=1:nrow(conflict_w), .combine = rbind, .options.snow = opts) %dopar% {
  ...
  neighborhood_o3_subset = conflict_w[which(conflict_w$Scenario==row_scenario & (conflict_w$iso3 %in% neighborhood_o3_iso3s) & conflict_w$year == row_year),]
...
fwrite(conflict_w_spatial, "intermediate_data/large/simple_uppsala_replication_forecasting.csv")
